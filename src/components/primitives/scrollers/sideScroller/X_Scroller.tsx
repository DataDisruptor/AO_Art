...
export default function X_Scroller({onImageViewChange} : any) {
  ...
  return (
    <div className="flex j-center">
        ...
        {images.map((img : string, index : number) => (
            <img onClick={() => onImageViewChange(index, true, images[index])} key={index} src={img} alt={`${index}`} width={window.innerWidth*0.25} style={{maxWidth: window.innerWidth/4, padding: window.innerWidth*0.005, cursor:'pointer'}} />
        ))}
        </div>
        {/* <div >
